...
def load_features_user() -> pd.DataFrame:
    df_user_data = batch_load_sql("""SELECT * FROM eduardstepanyan88_users_lesson_22""")
    df_user_data = df_user_data.drop(['index'], axis=1)
    return df_user_data


def load_features_post_control() -> pd.DataFrame:
    df_post_data = batch_load_sql("""SELECT * FROM eduardstepanyan88_posts_info_lesson_22""")
    df_post_data = df_post_data.drop(['index'], axis=1)
    return df_post_data


def load_features_post_test() -> pd.DataFrame:
    df_post_data = batch_load_sql("""SELECT * FROM eduardstepanyan88_posts_info_features_dl""")
    df_post_data = df_post_data.drop(['index'], axis=1)
    return df_post_data
